...
public class AuthenticatedAirlineManagerUpdateService extends AbstractGuiService<Authenticated, AirlineManager> {
	...
	public void load() {
		...
		object = this.repository.findOneAirlineManagerByUserAccountId(userAccountId);
	...
	public void validate(final AirlineManager object) {
		...
		Collection<AirlineManager> codigo = this.repository.findAirlineManagerByCode(cod).stream().filter(x -> x.getId() != object.getId()).toList();
